...
def sum_tree(t):
    ...
    15
    """
    return label(t) + sum([sum_tree(b) for b in branches(t)]) if not is_leaf(t) else label(t)
...
def balanced(t):
    ...
    for b in branches(t)[1:]:
        if not sum_tree(b) == sum_tree(refer):
            return False
...
def partial_reverse(s: list[int], start: int) -> None:
    ...
    rest = len(s) - start
    if rest % 2 != 0:
        for i in range(start, start + rest//2):
            s[i], s[2*(rest//2) + 2*start - i] = s[2*(rest//2) + 2*start - i], s[i]
    else:
        count = start + len(s) - 1
        for i in range(start, start + rest // 2):
