...
class Transaction:
    ...
    def report(self) -> str:
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            if self.before > self.after:
                return str(self.id) + ': ' + msg1 + str(self.before) + "->" + str(self.after)
            else:
                return str(self.id) + ': ' + msg2 + str(self.before) + "->" + str(self.after)
...
class Coin:
    ...
    def worth(self) -> int:
        ...
        result = self.cents + age - 50
        return result if result > 0 else self.cents
