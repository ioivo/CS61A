...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, f, count = 1, make_averaged(roll_dice,times_called), 1
    largest = f(num_rolls,dice)
    num_rolls += 1
    while num_rolls <= 10:
        current = f(num_rolls,dice)
        if current > largest:
            largest = current
            count = num_rolls
        num_rolls += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    later_score = sus_update(0, score, opponent_score)
    dis = later_score - score
    if dis >= threshold:
        return 0
    else:
        return num_rolls
